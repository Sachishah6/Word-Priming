...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\HP\\Documents\\psy310 lab in psyc\\word priming\\5.0 Word Priming_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        thisExp.addData('Study_Phase.started', globalClock.getTime(format='float'))
        StudyText.setText(Study_word)
        Studyslider.reset()
        # keep track of which components have finished
        Study_PhaseComponents = [StudyText, Studyslider]
        ...
        thisExp.addData('Study_Phase.stopped', globalClock.getTime(format='float'))
    ...
    for thisTrial_2 in trials_2:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Test_Phase.started', globalClock.getTime(format='float'))
        ...
        # keep track of which components have finished
        Test_PhaseComponents = [TestText, test_textbox, submit_text, testmouse]
        ...
        while continueRoutine:
            ...
            if TestText.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if test_textbox.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if submit_text.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if testmouse.status == NOT_STARTED and t >= 0-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in Test_PhaseComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Test_Phase.stopped', globalClock.getTime(format='float'))
        ...
        # the Routine "Test_Phase" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
